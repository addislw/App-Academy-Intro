...
def element_times_index(nums)
  return nums.map! { |num| num * nums.index(num) }
end
=end

def element_times_index(nums)
  new_nums = []
  nums.each_with_index { |ele, idx| new_nums << ele * idx }
  new_nums
end
...
def even_nums(max)
  nums_arr = []
  (0..max).each { |num| num % 2 == 0 ? nums_arr << num : next }
  nums_arr
end
...
def odd_range(min, max)
  return (min..max).select { |num| num % 2 != 0 }
end

# solution using inject (BROKEN)
=begin
def odd_range(min, max)
  num_arr = (min..max).inject([]) { |arr, num| arr << num if num % 2 != 0 }
end
=end
...
def first_half(array)
  new_arr = []
  length = array.length / 2.0

  array.each_with_index do |num, index|
    new_arr << num if index < length
  end
...
def select_long_words(words)
  new_words = words.select { |word| word.length > 4 }
end
